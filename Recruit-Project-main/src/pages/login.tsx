...
interface JwtPayload {
  id: string;
  role: string; // Có thể là "authenticated"
  user_metadata?: {
    role: string; // Vai trò cụ thể như "staff" hoặc "candidate"
  };
  iat?: number;
  exp?: number;
}
...
export default function LoginPage() {
  ...
  const handleCandidateLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userRole = decoded.user_metadata?.role || "candidate"; // Lấy từ user_metadata
  ...
  const handleHrLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("token", token);
      const decoded: JwtPayload = jwtDecode(token);
      const userRole = decoded.user_metadata?.role || "staff"; // Lấy từ user_metadata
      localStorage.setItem("userRole", userRole);
      // Dispatch sự kiện để thông báo thay đổi
      window.dispatchEvent(new Event("storageChange"));
      if (userRole === "staff") {
              ...
              >
                ...
                <form className="space-y-6" onSubmit={handleCandidateLogin}>
                  <motion.div className="grid gap-3" variants={itemVariants}>
                    <Label htmlFor="candidate-email" className="text-blue-800">
                      Email
                    </Label>
                  ...
                  <motion.div className="grid gap-3" variants={itemVariants}>
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setHrData({ ...hrData, staffCode: e.target.value })
                        }
                      />
                    </div>
                  </motion.div>
