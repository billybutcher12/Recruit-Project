...
export default function AdminPage() {
  ...
  const applicants: Applicant[] = [
    {
      ...
      cv_url: "/cvs/CV PHẠM HUY TÍN - Phạm Huy Tín-TopCV.vn.pdf",
    },
    {
      id: 2,
      name: "Trần Thị B",
  ...
  // Tính toán thống kê từ dữ liệu tĩnh
  const today = new Date().toISOString().split('T')[0];
  const todayApps = applicants.filter(app => app.created_at && app.created_at.startsWith(today));
  const monthApps = applicants.filter(app => app.created_at && app.created_at.startsWith(today.slice(0, 7)));
  const report = {
    today: todayApps.length,
    month: monthApps.length,
    passed: applicants.filter(app => app.status === 'Đậu').length,
    potential: applicants.filter(app => app.status === 'Chưa đánh giá').length,
    failed: applicants.filter(app => app.status === 'Trượt').length,
  };
  ...
  applicants.forEach(app => {
    const dayIndex = getDayOfWeek(app.created_at);
    chartData[dayIndex] += 1;
  });

  const totalApplicants = applicants.length;
  const chartPercentages = chartData.map(count => (count / totalApplicants) * 100);
  ...
  const handleExport = async (status: string) => {
    try {
      ...
      if (status === 'Đậu') {
        filteredApplicants = applicants.filter(app => app.status === 'Đậu');
      } else if (status === 'Trượt') {
        filteredApplicants = applicants.filter(app => app.status === 'Trượt');
      } else if (status === 'Chưa đánh giá') {
        filteredApplicants = applicants.filter(app => app.status === 'Chưa đánh giá');
      }

      const response = await fetch('http://localhost:5000/api/export-static-report', {
        method: 'GET',
      });
      ...
      link.href = url;
      link.setAttribute('download', `bao-cao-ung-vien-${status.toLowerCase()}.xlsx`);
  ...
  // Xử lý tải CV trực tiếp
  const handleDownloadCV = (cvUrl: string) => {
    if (cvUrl) {
      const link = document.createElement('a');
      link.href = cvUrl;
      link.setAttribute('download', ''); // Kích hoạt tải xuống trực tiếp
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } else {
      setError('Không tìm thấy file CV');
    }
  };
  ...
  return (
    <div className={`min-h-screen pt-16 ${theme}-theme`}>
              ...
              >
                <motion.div variants={itemVariants}>
                  <Card className="overflow-hidden border-blue-100 hover:shadow-md transition-all duration-300">
                    ...
                    <CardContent>
                      <div className="text-3xl font-bold text-blue-700">{report.today}</div>
                ...
                <motion.div variants={itemVariants}>
                  <Card className="overflow-hidden border-blue-100 hover:shadow-md transition-all duration-300">
                    ...
                    <CardContent>
                      <div className="text-3xl font-bold text-blue-700">{report.month}</div>
                      <div className="text-xs text-green-600 mt-1 flex items-center">
                        <ArrowUp className="h-3 w-3 mr-1" /> 8% so với tháng trước
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
                ...
                <motion.div variants={itemVariants}>
                  <Card className="overflow-hidden border-blue-100 hover:shadow-md transition-all duration-300">
                    ...
                    <CardContent>
                      <div className="text-3xl font-bold text-blue-700">{report.passed}</div>
                      <div className="text-xs text-green-600 mt-1 flex items-center">
                        <ArrowUp className="h-3 w-3 mr-1" /> 5% so với tháng trước
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
                ...
                <motion.div variants={itemVariants}>
                  <Card className="overflow-hidden border-blue-100 hover:shadow-md transition-all duration-300">
                    ...
                    <CardContent>
                      <div className="text-3xl font-bold text-blue-700">{report.potential}</div>
                      <div className="text-xs text-green-600 mt-1 flex items-center">
                        <ArrowUp className="h-3 w-3 mr-1" /> 15% so với tháng trước
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
                ...
                <Card className="mb-8 border-blue-100 hover:shadow-lg transition-all duration-300 overflow-hidden">
                  ...
                  <CardContent className="pt-6">
                    <div className="h-[250px] flex items-end justify-around">
                      {chartPercentages.map((height, index) => (
                              ...
                              <TableCell>
                                ...
                                >
                                  <Button
                                    ...
                                    onClick={() => handleDownloadCV(applicant.cv_url)}
                                  >
                        ...
                        <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                          ...
                          >
                            <Button
                              ...
                              onClick={() => handleExport('Chưa đánh giá')}
                            >
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div
                          className={`border border-blue-200 rounded-md p-4 flex items-center gap-2 cursor-pointer ${
